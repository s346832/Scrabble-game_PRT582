...
while my_timer > 0:
    ...
    print(Fore.RED + "You only have", my_timer, "seconds to play!")
    print(Fore.RESET + "Please enter a Word (", char_length, "alphabet) : ")
    ...
    if userInput.isalpha() and len(userInput) == char_length and my_timer > 0:
        ...
        if dictionary.check(userInput):
            ...
            print(Fore.LIGHTMAGENTA_EX, "Your Score is :", total_score, "and you finished in", 15-my_timer, "seconds")
        else:
            print(Fore.YELLOW + "Your input is not an English Word!")

    if userInput.isalpha() is False and my_timer > 0:
        print(Fore.YELLOW + "Your input is not alphabet!")

    if len(userInput) != char_length and userInput.isalpha() and my_timer > 0:
        print(Fore.YELLOW + "Length of your Word is not same with the requirement！", char_length)
